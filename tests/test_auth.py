...
class TestParseCurl:
    ...
    def test_missing_cookie_exits(self):
        curl = (
            "curl 'https://music.youtube.com/youtubei/v1/browse' "
            "-H 'authorization: Bearer ya29.abc123' "
            "-H 'x-goog-authuser: 0'"
    ...
    def test_missing_authorization_exits(self):
        curl = (
            "curl 'https://music.youtube.com/youtubei/v1/browse' "
            "-H 'x-goog-authuser: 0' "
    ...
    def test_missing_authuser_exits(self):
        curl = (
            "curl 'https://music.youtube.com/youtubei/v1/browse' "
            "-H 'authorization: Bearer ya29.abc123' "
            "-b 'CONSENT=YES; SID=abc'"
    ...
    def test_missing_multiple_fields_exits(self):
        with pytest.raises(SystemExit):
            parse_curl("curl 'https://music.youtube.com/youtubei/v1/browse'")

    def test_missing_fields_prints_error(self, capsys):
        with pytest.raises(SystemExit):
            parse_curl("curl 'https://music.youtube.com/youtubei/v1/browse'")
        out = capsys.readouterr().out
        assert "Error: Could not find:" in out

    def test_authuser_value_extracted(self):
        curl = (
            "curl 'https://music.youtube.com/youtubei/v1/browse' "
...
class TestMain:
    def test_no_curl_file_exits_zero(self, tmp_path, monkeypatch):
        monkeypatch.chdir(tmp_path)
        with pytest.raises(SystemExit) as exc_info:
            main()
        assert exc_info.value.code == 0

    def test_no_curl_file_prints_instructions(self, tmp_path, monkeypatch, capsys):
        monkeypatch.chdir(tmp_path)
        with pytest.raises(SystemExit):
            main()
        out = capsys.readouterr().out
    ...
    def test_creates_browser_json_when_missing(self, tmp_path, monkeypatch):
        monkeypatch.chdir(tmp_path)
        (tmp_path / "curl.txt").write_text(VALID_CURL)

        main()

        data = json.loads((tmp_path / "browser.json").read_text())
        assert data["cookie"] == "CONSENT=YES; SID=abc; HSID=def"
        assert data["authorization"] == "Bearer ya29.abc123"
        assert data["x-goog-authuser"] == "0"

    def test_merges_into_existing_browser_json(self, tmp_path, monkeypatch):
        monkeypatch.chdir(tmp_path)
        (tmp_path / "curl.txt").write_text(VALID_CURL)
    ...
    def test_overwrites_stale_auth_in_browser_json(self, tmp_path, monkeypatch):
        monkeypatch.chdir(tmp_path)
        (tmp_path / "curl.txt").write_text(VALID_CURL)
