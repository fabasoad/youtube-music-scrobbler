...
class TestGetDiffFromSnapshot:
    ...
    def test_no_overlap_returns_all_current(
        ...
        with open("last_snapshot.json", "w") as f:
            json.dump(
                [{"video_id": t.video_id, "title": t.title, "artists": t.artists,
                  "duration": t.duration, "album": t.album, "like_status": t.like_status,
                  "duration_seconds": t.duration_seconds, "thumbnail": t.thumbnail}
                 for t in snapshot], f
            )
    ...
    def test_roundtrip(
        ...
        original = [
            YouTubeMusicTrack(
                ...
                duration="3:00", album="Alb", like_status="LIKE",
                duration_seconds=180, thumbnail="https://example.com/t.jpg",
            )
        ]
